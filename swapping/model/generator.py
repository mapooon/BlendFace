...
class ADD(nn.Module):
    def __init__(self, h_inchannel, z_inchannel, z_id_size=256):
        ...
        self.conv_f = nn.Conv2d(h_inchannel, 1, kernel_size=1, stride=1, padding=0)
...
class ADDResBlock(nn.Module):
    def __init__(self, h_inchannel, z_inchannel, h_outchannel, z_id_size):
        ...
        self.add1 = ADD(h_inchannel, z_inchannel, z_id_size)
        self.add2 = ADD(h_inchannel, z_inchannel, z_id_size)
        ...
        if not self.h_inchannel == self.h_outchannel:
            self.add3 = ADD(h_inchannel, z_inchannel, z_id_size)
...
class ADDGenerator(nn.Module):
    def __init__(self, z_id_size, out_channels=3):
        ...
        self.h_outchannel = [1024, 1024, 1024, 512, 256, 128, 64, out_channels]

        self.model = nn.ModuleDict(
            {f"layer_{i}" : ADDResBlock(self.h_inchannel[i], self.z_inchannel[i], self.h_outchannel[i], z_id_size)
        for i in range(8)})
